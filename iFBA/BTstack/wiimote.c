...
#define NO_SQRT_MAG 1
//#define RIGHT_STICK_EMULATE_BUTTONS 1
...
int iOS_wiimote_check (struct  wiimote_t  *wm){
    ...
    if (IS_PRESSED(wm, WIIMOTE_BUTTON_UP))		{joyExKey |= WII_JOY_LEFT;}
    if (IS_PRESSED(wm, WIIMOTE_BUTTON_DOWN))	{joyExKey |= WII_JOY_RIGHT;}
    
    if (IS_PRESSED(wm, WIIMOTE_BUTTON_LEFT))	{
        if(!STICK2WAY &&
           !(STICK4WAY && (IS_PRESSED(wm, WIIMOTE_BUTTON_UP) ||
                           (IS_PRESSED(wm, WIIMOTE_BUTTON_DOWN)))))
            joyExKey |= WII_JOY_DOWN;
    }
    ...
    if (IS_PRESSED(wm, WIIMOTE_BUTTON_MINUS))	{joyExKey |= WII_JOY_SELECT;}
    if (IS_PRESSED(wm, WIIMOTE_BUTTON_PLUS))	{joyExKey |= WII_JOY_START;}
    if (IS_PRESSED(wm, WIIMOTE_BUTTON_ONE))		{joyExKey |= WII_JOY_A;}
    if (IS_PRESSED(wm, WIIMOTE_BUTTON_TWO))		{joyExKey |= WII_JOY_B;}
    if (IS_PRESSED(wm, WIIMOTE_BUTTON_HOME))	{joyExKey |= WII_JOY_HOME;}
    if (wm->exp.type == EXP_CLASSIC) {
        ...
        if (IS_PRESSED(cc, CLASSIC_CTRL_BUTTON_LEFT))		joyExKey |= WII_JOY_LEFT;
...
#endif
        {
            ...
            {
                if( v < 180){
                    joyExKey |= WII_JOY_RIGHT;
                    //printf("Right\n");
                }
                else if ( v >= 180){
                    joyExKey |= WII_JOY_LEFT;
                    //printf("Left\n");
                }
            }
            else if(STICK4WAY)
            ...
            else
            {
                if( v >= 330 || v < 30){
                    ...
                    joyExKey |= WII_JOY_UP;joyExKey |= WII_JOY_RIGHT;
                    //printf("UpRight\n");
                }
                else if ( v >= 60 && v < 120  ){
                    joyExKey |= WII_JOY_RIGHT;
                    ...
                    joyExKey |= WII_JOY_RIGHT;joyExKey |= WII_JOY_DOWN;
                    //printf("RightDown\n");
                }
                else if ( v >= 150 && v < 210  ){
                    joyExKey |= WII_JOY_DOWN;
                    ...
                    joyExKey |= WII_JOY_DOWN;joyExKey |= WII_JOY_LEFT;
                    //printf("DownLeft\n");
                }
                else if ( v >= 240 && v < 300  ){
                    joyExKey |= WII_JOY_LEFT;
        ...
        {                        
            ...
            if( v >= 330 || v < 30){
                ...
                joyExKey |= WII_JOY_A;joyExKey |= WII_JOY_C;
                //printf("Y B\n");
            }
            else if ( v >= 60 && v < 120  ){
                joyExKey |= WII_JOY_C;
                ...
                joyExKey |= WII_JOY_C;joyExKey |= WII_JOY_B;
                //printf("B X\n");
            }
            else if ( v >= 150 && v < 210  ){
                joyExKey |= WII_JOY_B;
                ...
                joyExKey |= WII_JOY_B;joyExKey |= WII_JOY_D;
                //printf("X A\n");
            }
            else if ( v >= 240 && v < 300  ){
                joyExKey |= WII_JOY_D;
                ...
                joyExKey |= WII_JOY_D;joyExKey |= WII_JOY_A;
                //printf("A Y\n");
            }
        }
