...
#if 1 && defined FBA_DEBUG
			printf(_T("Hi Score Memory Range %i Loaded from file\n"), i);
#endif
		}
...
void HiscoreReset()
{
	...
	if (!Debug_HiscoreInitted) printf(_T("HiscoreReset called without init\n"));
#endif
...
#if 1 && defined FBA_DEBUG
			printf(_T("Hi Score Memory Range %i Initted\n"), i);
#endif
		}
	}
}
...
{
#if defined FBA_DEBUG
	if (!Debug_HiscoreInitted) printf( _T("HiscoreApply called without init\n"));
#endif
...
#if 1 && defined FBA_DEBUG
				printf(_T("Applied Hi Score Memory Range %i on frame number %i\n"), i, GetCurrentFrame());
#endif
			} else {
				HiscoreMemRange[i].Applied = APPLIED_STATE_NONE;
				HiscoreMemRange[i].ApplyNextFrame = 1;
#if 1 && defined FBA_DEBUG
				printf(_T("Failed attempt to apply Hi Score Memory Range %i on frame number %i\n"), i, GetCurrentFrame());
#endif
			}
		}
...
{
#if defined FBA_DEBUG
	if (!Debug_HiscoreInitted) printf(_T("HiscoreExit called without init\n"));
#endif
