...
static void System16ARenderTileLayer(INT32 Page, INT32 PriorityDraw, INT32 Transparent)
{
	...
	UINT16 *pTileMapSrc = NULL,*pTileMapSrcY=NULL;
	UINT16 *pTileMapDest = NULL;
	INT32 xSrcOff, ySrcOff, RowScrollIndex, xEffScroll, yEffScroll;
	
	if (System16ColScroll && System16RowScroll) {
        if (Page == 0 && PriorityDraw == 0) pTileMapSrc = pSys16FgTileMapPri0;
        if (Page == 0 && PriorityDraw == 1) pTileMapSrc = pSys16FgTileMapPri1;
        if (Page == 1 && PriorityDraw == 0 && Transparent == 1) pTileMapSrc = pSys16BgTileMapPri0;
        if (Page == 1 && PriorityDraw == 1 && Transparent == 1) pTileMapSrc = pSys16BgTileMapPri1;
        if (Page == 1 && Transparent == 0) pTileMapSrc = pSys16BgTileMapOpaque;
        pTileMapDest = pTransDraw + (y * nScreenWidth);
        
        
		for (y = 0; y < nScreenHeight/2; y++) {
            ...
            if (Transparent) {
                ...
                for (x = 0; x < nScreenWidth; x++) {
                    yEffScroll = BURN_ENDIAN_SWAP_INT16(TextRam[0xf30/2 + (x/16) * 2 + Page]) & 0xff;
                    xSrcOff = (x - xEffScroll + 200) & 0x3ff;
                    pTileMapDest[x] = pTileMapSrcY[xSrcOff];
                }
            }
            pTileMapDest+=nScreenWidth;
		}
	} else {
		if (System16ColScroll) {
			...
			if (System16RowScroll) {
                ...
                if (Page == 0 && PriorityDraw == 0) pTileMapSrc = pSys16FgTileMapPri0;
                if (Page == 0 && PriorityDraw == 1) pTileMapSrc = pSys16FgTileMapPri1;
                if (Page == 1 && PriorityDraw == 0) pTileMapSrc = pSys16BgTileMapPri0;
                if (Page == 1 && PriorityDraw == 1) pTileMapSrc = pSys16BgTileMapPri1;
                if (Page == 1 && Transparent == 0) pTileMapSrc = pSys16BgTileMapOpaque;
				...
				for (y = 0; y < nScreenHeight; y++) {
					...
					if (Transparent) {
                        for (x = 0; x < nScreenWidth; x++) {	
                            Pix = pTileMapSrcY[(x - xScroll + 200) & 0x3ff];
                            if (Pix) pTileMapDest[x] = Pix;						
                        }
                    } else {
                        for (x = 0; x < nScreenWidth; x++) {
                            *pTileMapDest++ = pTileMapSrcY[(x - xScroll + 200) & 0x3ff];
                        }
                    }
				}
			}
...
static INT32 System16CalcPalette()
{
	...
	for (i = 0; i < System16PaletteEntries * 2; i +=2) {
		...
		System16Palette[i / 2] = HighCol16(r, g, b, 0);
		
		r = r * 160 / 256;
		g = g * 160 / 256;
		b = b * 160 / 256;
        
		System16Palette[(i / 2) + System16PaletteEntries] = HighCol16(r, g, b, 0);
		System16Palette[(i / 2) + (System16PaletteEntries * 2)] = HighCol16(r, g, b, 0);
	}
